  ...
  return (
    <section id="investimento" className="section-pad relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-5xl">
        ...
        <Reveal delay={0.1}>
          <div className="glass-card relative mb-8 overflow-hidden border-gold/40 p-8 ring-1 ring-gold/30 md:p-12">
            ...
            <div className="relative grid items-center gap-8 md:grid-cols-[1fr_auto]">
              <div>
                <span className="label-cinzel mb-4 inline-block rounded-sm border border-gold/40 px-4 py-2 text-[0.8rem] text-gold">
                  Condição Exclusiva
                </span>
                <h3 className="font-display text-4xl italic text-offwhite md:text-5xl">
              ...
              <div className="flex flex-col gap-6 md:items-end md:text-right">
                <div className="rounded-sm border border-gold/25 bg-background/40 px-7 py-5">
                  <p className="label-cinzel text-[0.75rem] text-gold">Até 20 de Agosto</p>
                  <p className="mt-2 font-display text-6xl text-gradient-gold leading-none">R$ 870</p>
                  <p className="mt-2 text-base text-muted-foreground">
                    3x de R$ 290 · parcelas a partir de 10/08
                  </p>
                </div>
                <div className="px-7">
                  <p className="label-cinzel text-[0.72rem] text-muted-foreground">
                    A partir de 21 de Agosto
                  </p>
                  <p className="mt-2 font-display text-3xl text-offwhite/90">
                    R$ 900{" "}
                    <span className="text-base font-body text-muted-foreground">· 3x de R$ 300</span>
                  </p>
                </div>
              </div>
            </div>
        ...
        <Reveal delay={0.12}>
          <div className="glass-card mb-12 p-8 md:p-10">
            <div className="text-center">
              <p className="label-cinzel text-base text-gold md:text-lg">Ingresso por dia</p>
              <p className="mt-3 text-lg text-muted-foreground">Escolha o dia do congresso</p>
            </div>
            <div className="mt-8 grid gap-5 sm:grid-cols-3">
              {dias.map((d) => (
                <div
                  key={d.dia}
                  className="rounded-sm border border-gold/20 bg-background/40 px-5 py-7 text-center"
                >
                  <p className="text-xl text-offwhite/90">{d.dia}</p>
                  <p className="mt-1 text-base text-muted-foreground">{d.data}</p>
                  <p className="mt-4 font-display text-5xl leading-none text-gradient-gold">
                    R$ {d.price}
                  </p>
                </div>
              ))}
            </div>
            <p className="mt-7 text-center text-base italic text-offwhite/80">
              Sem almoço incluído.
            </p>
          </div>
        </Reveal>
        ...
        <Reveal delay={0.15}>
          <p className="label-cinzel mb-7 text-center text-base tracking-[0.3em] text-gold md:text-lg">
            Público em geral
          </p>
        </Reveal>
          ...
          {lotes.map((l, i) => (
            <Reveal key={l.lote} delay={0.15 + i * 0.08}>
              <div className="glass-card flex h-full flex-col p-8 text-center">
                <p className="label-cinzel text-[0.8rem] text-gold">{l.lote}</p>
                <p className="mt-3 text-base text-muted-foreground">{l.period}</p>
                <div className="my-6 h-px w-12 self-center bg-gradient-to-r from-transparent via-gold/50 to-transparent" />
                <p className="font-display text-6xl leading-none text-offwhite">R$ {l.price}</p>
                <p className="mt-3 text-lg text-muted-foreground">{l.parcela}</p>
                {l.note && (
                  <p className="mt-4 text-sm italic text-muted-foreground/90">{l.note}</p>
                )}
              </div>
            </Reveal>
          ))}
        ...
        <Reveal delay={0.25}>
          <div className="glass-card mx-auto mt-12 max-w-3xl p-8 md:p-12">
            <div className="grid gap-10 md:grid-cols-[1fr_auto] md:items-center">
              <div>
                <p className="label-cinzel mb-5 text-base text-gold">Incluso em todos os lotes</p>
              ...
              <div className="text-center md:border-l md:border-gold/15 md:pl-10">
                <a
                  href="https://hotmart.com/pt-br/marketplace/produtos/iii-congresso-psigogenealogia-liz/Q106809712U"
                  target="_blank"
                  rel="noreferrer"
